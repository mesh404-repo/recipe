...
class TrainConfig:
    ...
    # EMA (Polyak averaging) of weights. 0 = off. Typical 0.999 = ~1000-step
    # window. Shadow copy maintained in float32 for numerical accuracy; swapped
    # into the model at final save time so the eval sees smoothed weights.
    ema_decay: float = 0.0
...
def train(cfg: TrainConfig, out_dir: Path, use_wandb: bool = False) -> dict:
    ...
    ema_shadow = None
    if cfg.ema_decay > 0:
        ema_shadow = {n: p.detach().clone().float() for n, p in model.named_parameters() if p.requires_grad}
        print(f"[train] EMA on: decay={cfg.ema_decay} start_step={cfg.ema_start_step} ({len(ema_shadow)} tensors)")
    ...
    for step in range(cfg.total_steps):
        ...
        if ema_shadow is not None and step >= cfg.ema_start_step:
            d = cfg.ema_decay
            with torch.no_grad():
                for n, p in model.named_parameters():
                    s = ema_shadow.get(n)
                    if s is not None:
                        s.mul_(d).add_(p.data.float(), alpha=1.0 - d)
    ...
    if ema_shadow is not None:
        ...
        print(f"[train] EMA weights swapped in for final checkpoint")
